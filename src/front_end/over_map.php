...
for (i = 0; i < acc.length; i++) {
  acc[i].onclick = function() {
    ...
    if (panel.style.maxHeight){
      ...
      var jsonObject = <?php include('../back_end/findHosts.php'); echo $myJsonObject; ?>;
      ...
      window.onload = function () {
        ...
        for (var j = 0; j < Object.keys(jsonObject).length-3; j++) {
          var temp = 'mybutton' + j;
        ...
        //on load of sideNav, create buttons
        for (var j = Object.keys(jsonObject).length-3; j < Object.keys(jsonObject).length; j++) {
          var temp = 'mybutton' + j;
          createButtons('button', jsonObject[j], 'mySidenav');
        }
        addBreak('mySidenav');
